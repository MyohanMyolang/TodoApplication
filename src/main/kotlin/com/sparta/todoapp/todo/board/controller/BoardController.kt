package com.sparta.todoapp.todo.board.controller
...
import com.sparta.todoapp.todo.board.service.BoardService
...
class BoardController(
	...
	@ApiResponses(
		ApiResponse(responseCode = "201", description = "생성 완료"), ApiResponse(
			responseCode = "400",
			...
			responseCode = "401",
			description = "로그인 되어 있지 않을 때 발생합니다.",
			content = [Content(mediaType = "application/json", schema = Schema(implementation = ErrorObject::class))]
		), ApiResponse(
			responseCode = "404",
			description = "로그인 상태가 정상적이지 않을 때 발생합니다.",
			content = [Content(mediaType = "application/json", schema = Schema(implementation = ErrorObject::class))]
		)
	)
	@PostMapping
	fun addTodoBoard(@RequestBody @Valid requestTodoBoardDto: RequestTodoBoardDto) = responseEntity(HttpStatus.CREATED) {
	...
	@ApiResponses(
		ApiResponse(responseCode = "200", description = "가져오기 완료"), ApiResponse(
			responseCode = "404", description = "page 범위 밖일 때 발생합니다.", content = [Content(
	...
	@ApiResponses(
		ApiResponse(responseCode = "200", description = "가져오기 완료"), ApiResponse(
			responseCode = "404", description = "해당 이름을 가진 Board가 존재하지 않을 때 발생합니다.", content = [Content(
				mediaType = "application/json",
