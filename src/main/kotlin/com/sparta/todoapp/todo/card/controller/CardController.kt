...
class CardController(
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "401", description = "로그인 되지 않았을 때 발생합니다.", content = [
                    Content(mediaType = "application/json", schema = Schema(implementation = ErrorObject::class))
                ]
            ),
            ApiResponse(
                responseCode = "403", description = "해당 유저의 Board가 아닐 때 발생합니다.", content = [
                    Content(mediaType = "application/json", schema = Schema(implementation = ErrorObject::class))
                ]
            ),
            ApiResponse(
                responseCode = "404", description = "board를 찾지 못하였을 때 발생합니다.",
                content = [
                    Content(mediaType = "application/json", schema = Schema(implementation = ErrorObject::class))
                ]
            )
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "404", description = "해당 Card를 찾지 못했을 때 발생합니다.", content = [
                    Content(mediaType = "application/json", schema = Schema(implementation = ErrorObject::class))
                ]
            ),
        ]
    ...
    fun getTodoCardListByIdWithCommentList(
        ...
        @RequestParam(required = false, defaultValue = "desc") @Pattern(
            regexp = "^(desc|asc)$",
            message = "sort는 desc 또는 asc 둘 중 하나의 값을 가져야 합니다."
        ) sort: String
    ) = responseEntity(HttpStatus.OK) {
